...
def Lathes_train_test_split(X, y, test_size, random_state):
    ...
    n_ids,unique_indices = np.unique(X[:,0],return_index=True)

    print(n_ids,unique_indices)

    target = []

    for indice in unique_indices:

        target.append(y[indice])

    train_ids, _ = train_test_split(n_ids, 
                        test_size=test_size, 
                        stratify=target, 
    ...
    for i in range(len(X)):
        ...
        if id in train_ids: train_idx.append(i)
